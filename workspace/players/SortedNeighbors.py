...
#STEP 1: Creation du méta graph
    ...
    def best_journeys_f(self, source: int, cheeses: List[int], graph: Dict[int, Dict[int, int]]) -> List[int]:
        """
        Trouve le chemin optimal passant par tous les fromages en optimisant l'ordre d'exploration.

        Args:
            source (int): Sommet de départ.
            cheeses (List[int]): Liste des sommets contenant des fromages.
            graph (Dict[int, Dict[int, int]]): Graphe représenté comme un dictionnaire où
                graph[u][v] est la distance entre les sommets u et v.

        Returns:
            List[int]: Liste des sommets dans l'ordre optimal pour ramasser tous les fromages.
        """
        best_journey = []
        ...
        def perma_rec(next_cheeses: List[int], current_journey: List[int], current_length: int):
            """
            Exploration récursive pour trouver le chemin optimal.

            Args:
                next_cheeses (List[int]): Liste des sommets restants à visiter.
                current_journey (List[int]): Chemin en cours d'exploration.
                current_length (int): Longueur totale du chemin parcouru jusqu'à présent.
            """
            nonlocal best_journey, best_length

            # Abandonner cette branche si elle dépasse déjà la meilleure solution connue
            if current_length >= best_length:
                return
            
            # Si tous les fromages ont été ramassés, mettre à jour la meilleure solution
            if not next_cheeses:
                if current_length < best_length:
            ...
            # Trier les sommets restants par distance croissante depuis le dernier sommet visité
            sorted_neighbors = sorted(next_cheeses, key=lambda v: graph[current_journey[-1]].get(v, float("inf")))


            # Explorer chaque sommet trié
            for i, next_vertex in enumerate(sorted_neighbors):
                perma_rec(
                    next_cheeses[:i] + next_cheeses[i + 1 :],  # Retirer le sommet visité
                    current_journey + [next_vertex],          # Ajouter le sommet au chemin
                    current_length + graph[current_journey[-1]].get(next_vertex, float("inf"))  # Mise à jour de la longueur
                )

        # Lancer l'exploration à partir du sommet source
