...
class GreedyEachCheese(Player):
    ...
    def preprocessing(self, maze: Maze, game_state: GameState) -> None:
        ...
        while len(cheese_location)>1:
            ...
            best_path, best_dist = self.greedy(cheese_location, initial_location, graphe)
            print("backtracking")
            ...
            print("best_path[0]", best_path[0])
            if best_path[0] in cheese_location:
                cheese_location.remove(best_path[0])
            initial_location = best_path[1]
